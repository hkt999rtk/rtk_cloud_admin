...
func TestPaymentClientMethodsPreserveTenantPathsHeadersAndResponses(t *testing.T) {
	seen := map[string]int{}
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Header.Get("Authorization") != "Bearer payment-access" {
		...
		w.Header().Set("Content-Type", "application/json")
		switch key {
		case "GET /v1/orgs/org%2Fsafe/billing/account":
			_, _ = w.Write([]byte(`{"account":{"id":"account-1","organization_id":"org/safe","currency":"TWD","available_balance_minor":50000,"state":"active","version":1},"auto_topup":null}`))
		case "GET /v1/orgs/org%2Fsafe/billing/ledger":
			if r.URL.Query().Get("limit") != "5" || r.URL.Query().Get("offset") != "2" {
				t.Fatalf("ledger query=%s", r.URL.RawQuery)
			}
			_, _ = w.Write([]byte(`{"ledger_entries":[{"id":"entry-1","direction":"debit","amount_minor":10000,"currency":"TWD","reason":"invoice_debit","balance_after_minor":50000}],"pagination":{"limit":5,"offset":2,"total":1}}`))
		case "GET /v1/orgs/org%2Fsafe/payment-methods":
			if r.URL.Query().Get("limit") != "3" {
				t.Fatalf("methods query=%s", r.URL.RawQuery)
			}
			_, _ = w.Write([]byte(`{"payment_methods":[{"id":"method-1","provider":"newebpay","status":"active","last_four":"4242","capabilities":{"hosted_setup":true}}],"pagination":{"limit":3,"offset":0,"total":1}}`))
		case "POST /v1/orgs/org%2Fsafe/payment-methods/setup":
			if r.Header.Get("Idempotency-Key") != "setup-key" {
				t.Fatalf("setup idempotency=%q", r.Header.Get("Idempotency-Key"))
			}
			var body struct {
			...
			if err := json.NewDecoder(r.Body).Decode(&body); err != nil || body.Provider != "newebpay" || !body.Consent.Accepted {
				t.Fatalf("setup body=%+v err=%v", body, err)
			}
			w.WriteHeader(http.StatusAccepted)
			_, _ = w.Write([]byte(`{"payment_method":{"id":"method-1"},"hosted_url":"https://payments.example/setup","duplicate":false}`))
		case "DELETE /v1/orgs/org%2Fsafe/payment-methods/method%2F1":
			_, _ = w.Write([]byte(`{"payment_method":{"id":"method/1","status":"revoked"},"duplicate":false}`))
		case "GET /v1/orgs/org%2Fsafe/auto-topup":
			w.Header().Set("ETag", `"7"`)
			_, _ = w.Write([]byte(`{"auto_topup":null}`))
		case "PUT /v1/orgs/org%2Fsafe/auto-topup":
			if r.Header.Get("If-Match") != `"7"` {
				t.Fatalf("put If-Match=%q", r.Header.Get("If-Match"))
			}
			w.Header().Set("ETag", `"8"`)
			_, _ = w.Write([]byte(`{"auto_topup":{"id":"policy-1","enabled":true,"version":8}}`))
		case "DELETE /v1/orgs/org%2Fsafe/auto-topup":
			if r.Header.Get("If-Match") != `"8"` {
				t.Fatalf("delete If-Match=%q", r.Header.Get("If-Match"))
			}
			w.Header().Set("ETag", `"9"`)
			_, _ = w.Write([]byte(`{"auto_topup":{"id":"policy-1","enabled":false,"version":9}}`))
		case "POST /v1/orgs/org%2Fsafe/topups":
			if r.Header.Get("Idempotency-Key") != "topup-key" {
		...
		case "GET /v1/orgs/org%2Fsafe/payment-intents":
			if r.URL.Query().Get("limit") != "4" {
				t.Fatalf("intents query=%s", r.URL.RawQuery)
			}
			_, _ = w.Write([]byte(`{"payment_intents":[{"id":"intent-1","state":"created"}],"pagination":{"limit":4,"offset":0,"total":1}}`))
		case "GET /v1/orgs/org%2Fsafe/payment-intents/intent%2F1":
			_, _ = w.Write([]byte(`{"payment_intent":{"id":"intent/1","state":"succeeded"},"attempts":[]}`))
		default:
			t.Fatalf("unexpected payment request: %s", key)
		}
	}))
	defer upstream.Close()
	...
	client := New(upstream.URL)
	ctx := t.Context()
	if result, err := client.BillingAccount(ctx, "payment-access", "org/safe"); err != nil || result.Account.ID != "account-1" {
		t.Fatalf("billing account=%+v err=%v", result, err)
	}
	if result, err := client.BillingLedger(ctx, "payment-access", "org/safe", url.Values{"limit": {"5"}, "offset": {"2"}}); err != nil || len(result.LedgerEntries) != 1 {
		t.Fatalf("ledger=%+v err=%v", result, err)
	}
	if result, err := client.PaymentMethods(ctx, "payment-access", "org/safe", url.Values{"limit": {"3"}}); err != nil || len(result.PaymentMethods) != 1 {
		t.Fatalf("methods=%+v err=%v", result, err)
	}
	setupRequest := map[string]any{"provider": "newebpay", "consent": map[string]any{"accepted": true}}
	if result, err := client.SetupPaymentMethod(ctx, "payment-access", "org/safe", "setup-key", setupRequest); err != nil || result["hosted_url"] == nil {
		t.Fatalf("setup=%+v err=%v", result, err)
	}
	if result, err := client.RevokePaymentMethod(ctx, "payment-access", "org/safe", "method/1", map[string]any{"reason": "customer request"}); err != nil || result["payment_method"] == nil {
		t.Fatalf("revoke=%+v err=%v", result, err)
	}
	if result, etag, err := client.AutoTopUp(ctx, "payment-access", "org/safe"); err != nil || result.AutoTopUp != nil || etag != `"7"` {
		t.Fatalf("auto topup=%+v etag=%q err=%v", result, etag, err)
	}
	if result, etag, err := client.PutAutoTopUp(ctx, "payment-access", "org/safe", `"7"`, map[string]any{"enabled": true}); err != nil || result.AutoTopUp == nil || etag != `"8"` {
		t.Fatalf("put auto topup=%+v etag=%q err=%v", result, etag, err)
	}
	if result, etag, err := client.DisableAutoTopUp(ctx, "payment-access", "org/safe", `"8"`, map[string]any{"reason": "pause"}); err != nil || result.AutoTopUp == nil || etag != `"9"` {
		t.Fatalf("disable auto topup=%+v etag=%q err=%v", result, etag, err)
	}
	if result, err := client.CreateManualTopUp(ctx, "payment-access", "org/safe", "topup-key", map[string]any{"amount_minor": 10000}); err != nil || result.PaymentIntent.ID != "intent-1" {
		t.Fatalf("manual topup=%+v err=%v", result, err)
	}
	if result, err := client.PaymentIntents(ctx, "payment-access", "org/safe", url.Values{"limit": {"4"}}); err != nil || len(result.PaymentIntents) != 1 {
		t.Fatalf("intents=%+v err=%v", result, err)
	}
	if result, err := client.PaymentIntent(ctx, "payment-access", "org/safe", "intent/1"); err != nil || result.PaymentIntent.ID != "intent/1" {
		t.Fatalf("intent=%+v err=%v", result, err)
	}
	if len(seen) != 11 {
		t.Fatalf("payment request inventory=%v", seen)
	}
}
