...
func TestClientFleetAndDeveloperWrappers(t *testing.T) {
	...
	_, err = client.DeveloperBrandCloudMemberInvitations(ctx, "access", "brand/1")
	check("DeveloperBrandCloudMemberInvitations", err)
	_, err = client.ResendDeveloperBrandCloudMemberInvitation(ctx, "access", "brand/1", "invitation/1")
	check("ResendDeveloperBrandCloudMemberInvitation", err)
	_, err = client.CancelDeveloperBrandCloudMemberInvitation(ctx, "access", "brand/1", "invitation/1")
	check("CancelDeveloperBrandCloudMemberInvitation", err)
	_, _, err = client.AcceptDeveloperBrandCloudMemberInvitation(ctx, "access", "invitation-token")
	check("AcceptDeveloperBrandCloudMemberInvitation", err)
	...
	_, err = client.ProductCollaborators(ctx, "access", "brand/1", "product/1")
	check("ProductCollaborators", err)
	_, err = client.ProductCollaboratorInvitations(ctx, "access", "brand/1", "product/1")
	check("ProductCollaboratorInvitations", err)
	_, err = client.InviteProductCollaborator(ctx, "access", "brand/1", "product/1", "collaborator@example.com", "product_editor")
	check("InviteProductCollaborator", err)
	_, err = client.ActOnProductCollaboratorInvitation(ctx, "access", "brand/1", "product/1", "invitation/1", "cancel")
	check("ActOnProductCollaboratorInvitation", err)
	_, err = client.UpdateProductCollaborator(ctx, "access", "brand/1", "product/1", "user/1", "product_viewer")
	check("UpdateProductCollaborator", err)
	check("RemoveProductCollaborator", client.RemoveProductCollaborator(ctx, "access", "brand/1", "product/1", "user/1"))
	check("TransferProductOwnership", client.TransferProductOwnership(ctx, "access", "brand/1", "product/1", "user/1"))
	_, err = client.AcceptProductCollaboratorInvitation(ctx, "access", "invitation-token")
	check("AcceptProductCollaboratorInvitation", err)
