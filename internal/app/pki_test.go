...
func TestReplacementReconciliationProxyRequiresSessionAndSameOrigin(t *testing.T) {
	calls := 0
	upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		calls++
		if r.URL.Path != "/v1/platform/pki/issuers/00000000-0000-0000-0000-000000000001/reconcile-replacement" || r.Header.Get("Authorization") != "Bearer access" || r.Header.Get("Idempotency-Key") != "pki-test" {
			t.Errorf("wrong proxy identity/path")
		}
		w.Header().Set("Content-Type", "application/json")
		w.Write([]byte(`{"items":[]}`))
	...
	for _, tc := range []struct {
		...
		req := developerPKIRequest(t, tc.session, "/api/platform/pki/issuers/00000000-0000-0000-0000-000000000001/reconcile-replacement", "pki-test", `{"request_id":"renewal","serial_number":"ab:cd"}`)
		req.Header.Set("Content-Type", "application/json")
		req.Header.Set("Origin", tc.origin)
		rec := httptest.NewRecorder()
		srv.ServeHTTP(rec, req)
...
func TestLegacyMigrationProxyRequiresSessionAndSameOrigin(t *testing.T) {
	for _, path := range []string{"/issuers/00000000-0000-0000-0000-000000000001/legacy-import", "/operations/00000000-0000-0000-0000-000000000001/legacy-import"} {
		t.Run(path, func(t *testing.T) {
			calls := 0
			upstream := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				calls++
				if r.URL.Path != "/v1/platform/pki"+path || r.Header.Get("Authorization") != "Bearer access" || r.Header.Get("Idempotency-Key") != "pki-test" {
					t.Error("wrong proxy identity/path")
				}
				w.Header().Set("Content-Type", "application/json")
			...
			for _, tc := range []struct {
				...
				req := developerPKIRequest(t, tc.session, "/api/platform/pki"+path, "pki-test", `{}`)
				req.Header.Set("Content-Type", "application/json")
				req.Header.Set("Origin", tc.origin)
				rec := httptest.NewRecorder()
				srv.ServeHTTP(rec, req)
			...
			if calls != 1 {
				t.Fatalf("unauthorized upstream call count %d", calls)
			}
		})
	}
}
