...
The built-in Product capability codes are `video_streaming`, `video_storage`,
`mqtt`, and `iot_shadow`, displayed as `Live`, `Recording Storage`, `MQTT`, and
`IoT Shadow`. The selectable set comes from the live service catalog and may
include third-party registered option codes; the BFF must not restrict it to
these built-ins. Product writes that change options send the catalog revision.
`iot_shadow` is independently configurable and must not be inferred from
`mqtt`: HTTP Shadow requires `iot_shadow`, while MQTT Shadow requires both
`iot_shadow` and `mqtt`. Product list capability cells must not render human
management permissions such as Manage Devices or Edit Product.
