...
| Device telemetry | `GET /api/developer/brand-clouds/{cloudId}/devices/{id}/telemetry` | implemented / live evidence pending | Explicit-cloud DTO, Video Cloud proxy mode, customer-safe redaction, unauthorized/unavailable handling, and isolation tests exist. Production sign-off still requires authoritative per-device telemetry from a live Video Cloud environment. |
| Fleet health | `GET /api/developer/brand-clouds/{cloudId}/fleet/health-summary` | implemented / live evidence pending | BFF proxies Video Cloud PostgreSQL-backed health aggregation, including source freshness, and fails unavailable without demo fallback. Dev sign-off still requires live virtual-device evidence. |
| Fleet stream | `GET /api/developer/brand-clouds/{cloudId}/fleet/stream-stats` | implemented / live evidence pending | BFF route proxies Video Cloud WebRTC session stats and handles upstream failures. Production sign-off requires live WebRTC session event aggregation evidence. |
| Firmware | `GET /api/developer/brand-clouds/{cloudId}/fleet/firmware-distribution` | implemented / live evidence pending | BFF route proxies firmware endpoints when configured and avoids treating generated fallback versions as production evidence. Production sign-off requires observed firmware and rollout facts. |
...
| Fleet summary | `GET /api/developer/brand-clouds/{cloudId}/fleet/summary` | implemented / live evidence pending | Account Manager aggregates status, Product, model, firmware, region, enabled services, and per-Product region/firmware distributions in the database. Group/job/authoritative health dimensions remain separate indexed sources. |
| CSV provisioning | `POST /api/developer/brand-clouds/{cloudId}/provisioning/{sources,validate,jobs}` | implemented / dev evidence pending | Seven-day immutable checksum-bound source, row validation job, explicit execution confirmation, idempotent replay, result downloads and cross-cloud rejection are implemented. Dev evidence is blocked until deployment preflight has the operator-confirmed LKE service limit. |
...
| Groups and tags | List, create, update, member counts, and use as batch/OTA scope | P1 implementation adds the Account Manager tag catalog, scoped Cloud Admin CRUD, and asynchronous batch integration. Indexed group aggregate source fields remain a separate follow-up if upstream telemetry does not expose them |
...
| Reports | Filtered report generation and asynchronous export | P1 implementation adds complete report download metadata and an S3-compatible object-storage adapter with short-lived signed URLs; local MinIO wiring and full restart E2E remain validation work |
