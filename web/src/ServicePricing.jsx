...
export function ServicePricing({ tabs }) {
  ...
  return <section className="page-content billing-page service-pricing-page" data-testid="billing-pricing-page">
    <div className="page-intro"><div><p className="eyebrow">Realtek Managed Cloud</p><h2>Service pricing</h2><p>Explore proposed rates for messaging, device state, video and cloud operations.</p></div><span className="pricing-draft">TWD pricing proposal · 24 Sep 2026</span></div>
    {tabs}
    <section className="pricing-cost-summary" aria-labelledby="pricing-cost-heading">
      <div className="pricing-cost-total"><p className="eyebrow">Example monthly total</p><h3 id="pricing-cost-heading">Your cost at a glance</h3><strong className="pricing-cost-value">NT${(examplePublish + exampleDelivery + exampleShadow).toLocaleString('en-US')}</strong><span>TWD / month · before tax</span><p>Illustrative estimate for the usage shown. This is not your Cloud’s actual spend or an invoice.</p></div>
      <div className="pricing-cost-breakdown"><h4>Usage behind this estimate</h4><dl><div><dt>1 million MQTT publishes</dt><dd>NT${examplePublish}</dd></div><div><dt>5 million MQTT deliveries</dt><dd>NT${exampleDelivery}</dd></div><div><dt>1 million Shadow units over HTTP</dt><dd>NT${exampleShadow}</dd></div></dl><p>Includes these three usage items. Video, storage and other services are additional when used.</p></div>
    </section>
    <section className="pricing-intro" aria-label="Service rates and billing basis">
      <div><h3>Service rates &amp; billing basis</h3><p>Explore the unit prices and counting rules below. These TWD rates are proposals for review, before tax.</p></div>
      <dl><div><dt>Currency</dt><dd>{pricingCurrency} · NT$</dd></div><div><dt>Basis</dt><dd>Monthly usage</dd></div><div><dt>Proposal date</dt><dd><time dateTime={pricingProposalDate}>24 Sep 2026</time></dd></div></dl>
    </section>
    ...
    <div className="pricing-table-wrap"><table className="pricing-table">
      ...
      <tbody>{rows.map(row => <tr key={row.id}>
        ...
        <td data-label="Proposed price" className="pricing-rate"><strong>NT${row.price.toLocaleString('en-US', { maximumFractionDigits: 2 })}</strong><span>/ {row.unit}</span></td>
    ...
    <section className="panel pricing-methodology"><h3>How to read this rate card</h3><ul>
      ...
      <li>Public benchmarks retain their source currency, usually USD. RTK's proposed customer rates are in TWD and are not provider invoices. The planning conversion is fixed at US$1 = NT$32; no exchange conversion occurs when invoicing.</li>
