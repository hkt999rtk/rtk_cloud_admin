...
export function CloudProducts({cloudId,productId='',onAccessLost}) {
  ...
  function edit(p) {
    ...
    setForm(p?{id:p.id,name:p.name,profile_key:p.profile_key,product_model:p.product_model||'',category:p.category,service_options:p.service_options,original_services:p.service_options,log_retention_days:p.log_retention_days||7,original_log_retention_days:p.log_retention_days||7}:
  ...
  async function write(event) {
    ...
    const servicesChanged=!form?.id||JSON.stringify([...form.service_options].sort())!==JSON.stringify([...form.original_services].sort())||(form.service_options.includes('device_logging')&&form.log_retention_days!==form.original_log_retention_days);
    ...
    const body=disable?{}:{name:form.name,product_model:form.product_model,category:form.category,...(servicesChanged?{service_options:form.service_options,...(catalog?.product_writes_enabled?{catalog_revision:catalog.catalog_revision}:{})}:{}),...(servicesChanged&&form.service_options.includes('device_logging')?{log_retention_days:form.log_retention_days}:{}),...(!form.id?{profile_key:form.profile_key}:{})};
  ...
  return <section className="my-clouds-panel cloud-products" data-testid="cloud-products">
    ...
    {productId && data?.products[0] && <>{data.products[0].allowed_actions?.includes('edit')&&<ProductServiceApply cloudId={cloudId} product={data.products[0]} catalog={catalog} canStart={catalog?.product_writes_enabled===true}/>}<DeviceEnrollmentGuide cloudId={cloudId} product={data.products[0]}/><ProductionRuns cloudId={cloudId} product={data.products[0]}/></>}
    {data?.products.length > 0 && <div className="ui-table-scroll"><table><caption>{productId ? translate("Product configuration") : translate("Products in your authorized scope")}</caption><thead><tr><th>{translate("Product")}</th><th>{translate("Status")}</th><th>{translate("Model / category")}</th><th>{translate("Services")}</th><th>{translate("Access")}</th><th>{translate("Actions")}</th></tr></thead><tbody>{data.products.map(p=><tr key={p.id}><td><a href={productURL(cloudId,p.id)}>{p.name}</a><small>{productId ? <CopyValue value={p.profile_key} label="product key"/> : p.profile_key}</small></td><td><StatusBadge value={p.status}/><PKIStatus value={p.pki_status}/></td><td>{p.product_model||translate("Not specified")}<small>{displayLabel(p.category)}</small></td><td>{p.service_options.map(displayLabel).join(', ')||translate("None")}{p.log_retention_days&&<small>{translate("Logs:")} {p.log_retention_days} {translate("days")}</small>}{p.grant_revision>0&&<small>{translate("Authorization version")} {p.grant_revision}</small>}</td><td>{displayLabel(p.my_role||'Cloud-scoped access')}</td><td><div className="my-clouds-actions">{p.allowed_actions?.includes('edit') && <button disabled={busy} onClick={()=>edit(p)}>{translate("Edit Product")}</button>}{p.status==='active' && p.allowed_actions?.includes('disable') && <button disabled={busy} onClick={()=>{intent.current=null;setForm(null);setDisable(p);}}>{translate("Disable Product")}</button>}</div></td></tr>)}</tbody></table></div>}
