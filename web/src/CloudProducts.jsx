...
export function CloudProducts({cloudId,productId='',onAccessLost}) {
  ...
  function edit(p) {
    ...
    setForm(p?{id:p.id,name:p.name,profile_key:p.profile_key,product_model:p.product_model||'',category:p.category,service_options:p.service_options,original_services:p.service_options}:
      {id:'',name:'',profile_key:'',product_model:'',category:'ip_camera',service_options:['mqtt'],original_services:[]});
  }
  async function write(event) {
    ...
    const body=disable?{}:{name:form.name,product_model:form.product_model,category:form.category,...(servicesChanged?{service_options:form.service_options,...(catalog?.product_writes_enabled?{catalog_revision:catalog.catalog_revision}:{})}:{}),...(!form.id?{profile_key:form.profile_key}:{})};
