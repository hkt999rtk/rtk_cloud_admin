...
export function MyCloudsApp() {
  ...
  return <CloudConsoleShell me={me} cloud={cloud} clouds={page?.brand_clouds || me?.memberships || []} active={shellActive} title={cloudId ? cloud?.name || 'Brand Cloud' : 'My Clouds'} onError={setError}>
    <div className="my-clouds-main">
      ...
      {cloud && section === 'settings' && !operation && <section className="my-clouds-panel"><h2>Cloud settings</h2><dl><dt>Cloud name</dt><dd>{cloud.name}</dd><dt>Description</dt><dd>{cloud.description || 'No description'}</dd><dt>Cloud ID</dt><dd>{cloud.id}</dd><dt>Tenant slug</dt><dd>{cloud.tenant_slug || 'Unavailable'}</dd><dt>Owner email</dt><dd>{ownerAccountEmail(cloud, me)}</dd><dt>Owner ID</dt><dd>{cloud.owner_user_id || 'Unavailable'}</dd><dt>My role</dt><dd>{cloud.my_role}</dd></dl>{lifecycleWarning(cloud.status) && <p className="my-clouds-lifecycle-warning" role="status">{lifecycleWarning(cloud.status)}</p>}{canManage && <div className="my-clouds-actions"><button disabled={busy} onClick={() => { intent.current = null; setForm({ id: cloud.id, name: cloud.name, description: cloud.description }); }}>Edit cloud</button><button className="my-clouds-danger" disabled={busy} onClick={checkDeletion}>Check deletion</button></div>}</section>}
