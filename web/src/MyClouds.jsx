...
import { cloudConsolePath } from './cloud-scope.mjs';
...
export function MyCloudsApp() {
  ...
  return <div className="my-clouds-shell">
    ...
    <main className="my-clouds-main">
      ...
      {cloud && !productId && !operation && <nav className="my-clouds-tabs" aria-label="Cloud services"><a href={cloudConsolePath(cloudId, 'devices')}>Fleet</a><a href={cloudConsolePath(cloudId, 'groups')}>Groups &amp; Tags</a><a href={cloudConsolePath(cloudId, 'provisioning')}>Provisioning</a><a href={cloudConsolePath(cloudId, 'firmware-ota')}>Firmware OTA</a><a href={cloudConsolePath(cloudId, 'stream-health')}>Streaming Health</a><a href={cloudConsolePath(cloudId, 'reports')}>Reports</a>{cloud.my_role === 'owner' && cloud.capabilities?.includes('billing_account.read') ? <a href={cloudConsolePath(cloudId, 'billing')}>Billing</a> : null}</nav>}
