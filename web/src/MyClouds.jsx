...
export function MyCloudsApp() {
  ...
  return <CloudConsoleShell me={me} cloud={cloud} clouds={page?.brand_clouds || me?.memberships || []} active={shellActive} title={cloudId ? cloud?.name || 'Brand Cloud' : 'My Clouds'} onError={setError}>
    <div className="my-clouds-main">
      <div className="my-clouds-heading"><div><p className="my-clouds-eyebrow">DEVELOPER CONSOLE</p><p>{cloudId ? cloudIntroduction(section) : 'Create and manage the Brand Clouds you own, or open clouds shared with your account. Select a cloud to work with its Products, Fleet Management, firmware, members, settings, and owner-only Billing.'}</p></div>{!cloudId && page && <button className="icon-text" disabled={!canCreate || busy} onClick={() => { intent.current = null; setForm({ id: '', name: '', description: '' }); }}><SemanticIcon name="plus" />Create cloud</button>}</div>
      ...
      {!cloudId && <>
        <div className="my-clouds-tabs" role="group" aria-label="Cloud visibility">{[['all', 'All clouds'], ['owned', 'I own'], ['shared', 'Shared with me']].map(([id, label]) => <button key={id} aria-pressed={view === id} onClick={() => { setView(id); setOffset(0); }}>{label}</button>)}</div>
        {page && <p className="my-clouds-quota">Owned: {page.owned_count} / {page.owned_limit} · Reserved for transfers: {page.reserved_count} · Matching clouds: {page.pagination.total}</p>}
        {loading ? <p role="status">Loading clouds…</p> : page && <>
          {page.brand_clouds.length === 0
            ? <section className="my-clouds-panel"><h2>No clouds in this view</h2><p>Create a cloud or accept an owner’s invitation. Shared clouds do not use your ownership quota.</p></section>
            : <div className="my-clouds-grid">{page.brand_clouds.map((item) => <article className="my-clouds-panel" key={item.id}>
              ...
              <div className="my-clouds-actions">
                <a className="my-clouds-open-action icon-text" href={cloudURL(item.id)}><SemanticIcon name="arrow-right" />Open cloud</a>
                {item.capabilities.includes('cloud.update') && <button className="icon-text" onClick={() => { intent.current = null; setForm({ id: item.id, name: item.name, description: item.description }); }}><SemanticIcon name="pen-to-square" />Edit</button>}
              </div>
            </article>)}</div>}
          {page.pagination.total > page.pagination.limit && <nav className="my-clouds-pagination" aria-label="Cloud pages"><button disabled={offset === 0} onClick={() => setOffset(Math.max(0, offset - 25))}>Previous</button><span>Page {Math.floor(offset / 25) + 1}</span><button disabled={offset + 25 >= page.pagination.total} onClick={() => setOffset(offset + 25)}>Next</button></nav>}
        </>}
      </>}
