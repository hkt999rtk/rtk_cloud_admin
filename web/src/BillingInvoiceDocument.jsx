...
export function BillingInvoiceDocument({ invoice = SAMPLE_INVOICE, preview = false }) {
  ...
  return <article className="billing-invoice-document" aria-label={preview ? 'Sample invoice' : 'Invoice document'} data-testid={preview ? 'invoice-preview' : 'invoice-document'}>
    ...
    <header className="invoice-document-header">
      ...
      <div className="invoice-document-amount"><span>{preview ? 'Example total before tax' : 'Invoice total'} ({invoice.currency})</span><strong>{money(invoice.total_minor)}</strong><span>{preview ? 'Tax is calculated only when invoiced' : 'Includes recorded tax'}</span></div>
    </header>
    ...
    <div className="invoice-document-summary">
      <div className="invoice-document-note"><h4>{preview ? 'About this preview' : 'About this invoice'}</h4><p>{preview ? 'Example quantities and prices demonstrate the layout, not your usage or current service rates. Tax is not calculated in this preview.' : 'Amounts and recipient details reflect the issued invoice snapshot. Updating your billing profile does not change this invoice.'}</p></div>
      <dl className="invoice-document-totals"><div><dt>Subtotal (before tax)</dt><dd>{money(invoice.subtotal_minor)}</dd></div>{!preview && <div><dt>Tax (as issued)</dt><dd>{money(invoice.tax_minor)}</dd></div>}<div className="invoice-document-grand-total"><dt>{preview ? 'Example total before tax' : 'Total (incl. tax)'}</dt><dd>{money(invoice.total_minor)}</dd></div>{!preview && <><div><dt>Settled from balance</dt><dd>{money(invoice.amount_settled_minor)}</dd></div><div><dt>Amount due</dt><dd>{money(invoice.amount_due_minor)}</dd></div></>}</dl>
    </div>
