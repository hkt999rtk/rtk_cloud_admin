...
export function TestLabDevices({ cloudId, product, onScope }) {
  ...
  return <section className="test-lab-device-manager" aria-label={translate("Test account and device bindings")}>
    ...
    <div className="test-lab-actions"><button disabled={busy || !product} onClick={createDevice}>{icon('plus')}{translate("Create test device")}</button><button disabled={busy || !account || !product} onClick={() => setShowBind(v => !v)}>{icon('link')}{translate("Bind device")}</button></div>
    {showBind && <div className="test-lab-binding-form"><label>{translate("Unbound test device")}<select value={candidate} onChange={e => setCandidate(e.target.value)}><option value="">{translate("Select a test device")}</option>{devices.filter(d => d.bindable).map(d => <option key={d.id} value={d.id}>{d.name} — {d.id}</option>)}</select></label><p>{translate("Only server-verified test-issued devices are eligible. Binding uses a fresh, developer-approved test authorization, not a production claim override.")}</p><button disabled={busy || !candidate} onClick={bind}>{translate("Confirm bind")}</button></div>}
    <h3>{icon('microchip')}{translate("Bound devices")}</h3><p>{translate("Bound, provisioned and online are separate states. Load the downloaded device credentials into your test board or device client to connect it.")}</p>
    {!account ? <p>{translate("Your bound test devices will appear automatically.")}</p> : !devices.some(d => d.bound) ? <p>{translate("No bound test devices in this Product. Create or bind a test device to begin.")}</p> : <div className="table-wrap"><table><thead><tr><th>{translate("Device")}</th><th>{translate("Provision")}</th><th>{translate("Connection")}</th><th>{translate("Actions")}</th></tr></thead><tbody>{devices.filter(d => d.bound).map(d => <tr key={d.id}><td>{d.name}<small className="test-lab-device-id">{d.id}</small></td><td>{d.provision_status}</td><td>{d.connection_status === 'online' || d.connection_status === 'offline' ? d.connection_status : translate("Unknown")}</td><td><button disabled={busy} aria-pressed={selected === d.id} onClick={() => setSelected(d.id)}>{selected === d.id ? translate("Selected") : translate("Select")}</button>{!['activated', 'pending'].includes(d.provision_status) && <button disabled={busy} onClick={() => { setProvision({ ...d, operation: crypto.randomUUID(), activity: crypto.randomUUID() }); setPublicKey(''); }}>{translate("Provision")}</button>}<button disabled={busy || d.provision_status === 'pending'} onClick={() => unbind(d)}>{icon('link-slash')}{translate("Unbind")}</button></td></tr>)}</tbody></table></div>}
