...
export function ProvisioningPage({ products = [], canCreate = false }) {
  ...
  return <section className="page-content">
    <div className="page-intro"><div><p className="eyebrow">Fleet Operations</p><h2>CSV Provisioning</h2><p>Upload a device CSV, review validation results, then confirm provisioning.</p></div></div>
    <ol className="ui-steps" aria-label="Provisioning progress"><li aria-current={!source ? 'step' : undefined}>Upload CSV</li><li aria-current={source && !validationReady ? 'step' : undefined}>Validate devices</li><li aria-current={validationReady ? 'step' : undefined}>Confirm and monitor</li></ol>
    {!canCreate ? <section className="panel"><h3>Read-only access</h3><p>Your role can review provisioning jobs but cannot upload or provision devices.</p></section> : <section className="panel"><form className="report-builder" onSubmit={upload}>
      <label>Product<select aria-label="Product" required value={productId} onChange={(event) => { setProductId(event.target.value); setSource(null); intent.current = crypto.randomUUID(); }}><option value="">Select Product</option>{products.map((product) => <option key={product.id} value={product.id}>{product.name}</option>)}</select></label>
      <label>Production run (optional)<input value={productionRun} onChange={(event) => setProductionRun(event.target.value)} placeholder="e.g. September production" /></label>
      <label>Device CSV<input required type="file" accept=".csv,text/csv" onChange={(event) => { setFile(event.target.files?.[0] || null); setSource(null); intent.current = crypto.randomUUID(); }} /><small>Include a device_id column. Validate the uploaded file before execution.</small></label>
      <button className="primary-button" disabled={busy || !file || !productId}>Upload source</button>
    </form></section>}
    {source ? <section className="panel"><h3>Source</h3><p>{source.filename} · {source.row_count} rows · expires {source.expires_at}</p><details><summary>Source integrity</summary><code>{source.checksum}</code></details><div className="report-builder-actions"><button disabled={busy || Boolean(validation)} onClick={() => start('validation')}>Validate rows</button>{validationReady ? <button className="primary-button" disabled={busy || Boolean(execution)} onClick={() => start('execution')}>Confirm provisioning</button> : null}</div></section> : null}
