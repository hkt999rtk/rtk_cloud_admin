    ...
    <section className="page-content">
      ...
      {showCreate ? <section className="panel"><form className="product-create-form" onSubmit={createProduct}>
        ...
        <div className="service-checks"><p>{translate("{{count}} of 64 selected",{count:form.service_capabilities.length})}</p>{serviceChoices.map((service) => <label key={service.code}>
          ...
          {translate(service.display_name || service.code)}{!service.selectable ? ` (${translate(productServiceAvailability(service))})` : ''}{service.requires?.length ? translate(" — requires {{value0}}", { value0: service.requires.join(', ') }) : ''}{service.description ? <small>{translate(service.description)}</small> : null}
        </label>)}</div>
          ...
          <div className="table-wrap">
            <table className="data-table">
              ...
              <tbody>{items.map((product) => <tr key={product.id}>
                ...
                <td>{product.service_capabilities?.length ? product.service_capabilities.map((code) => productServiceCapabilityLabel(code, catalog)).join(', ') : translate("Inactive")}{product.service_capabilities?.includes('device_logging') ? translate(" · Logs {{value0}} days", { value0: product.log_retention_days || 7 }) : ''}{product.grant_revision ? <small>{translate('Authorization version')} {product.grant_revision}</small> : null}</td>
      ...
      {applyProduct?.brand_cloud_id&&<section className="panel"><button type="button" className="link-button" onClick={()=>setApplyProduct(null)}>{translate('Close')}</button><ProductServiceApply cloudId={applyProduct.brand_cloud_id} product={applyProduct} catalog={catalog} canStart={catalog?.product_writes_enabled===true}/></section>}
