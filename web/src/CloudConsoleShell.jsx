...
export function CloudConsoleShell({ me, cloud = null, clouds = [], active = 'my-clouds', title = '', children, onError = () => {} }) {
  ...
  const groups = cloudShellNavGroups(cloudId, capabilities, { isOwner, showOwnerOnly });
      ...
      <nav className="sidebar-nav-groups">
        {groups.map((group) => <section className="sidebar-nav-group" key={group.id}>
          ...
          {group.items.map((item) => item.disabled
            ...
            : <a key={item.id} className={item.id === active ? 'active' : ''} aria-current={item.id === active ? 'page' : undefined} href={cloudConsolePath(cloudId, item.id)} onClick={() => setMobileOpen(false)}>{icon(item.icon)}{item.labelKey}</a>)}
        </section>)}
      </nav>
