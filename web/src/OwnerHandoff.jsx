...
export function OwnerHandoffPage() {
  ...
  return <div className="my-clouds-shell owner-handoff"><header className="my-clouds-header"><a href="/console/clouds">{translate("Realtek Connect / My Clouds")}</a></header><main className="my-clouds-main">
    <h1>{translate("Ownership and Billing handoff")}</h1><p>{translate(policy)}</p>
    {error && <div role="alert"><p>{translate(error)} <a href={loginPathFor(loginNext)}>{translate("Sign in again")}</a></p><button disabled={busy} onClick={() => { setPreviewKey(''); setApproval(''); setReload(value => value + 1); }}>{translate("Retry account and handoff status")}</button></div>}
    {!me && !error && <p role="status">{translate("Loading account…")}</p>}
    {route?.accept && me && <section className="my-clouds-panel owner-handoff"><h2>{translate("Accept ownership invitation")}</h2><p>{translate("Signed in as")} {me.email}{translate(". Only the invited global account can accept. Acceptance begins settlement; each party must later confirm the same exact balance.")}</p>{!token && <p role="alert">{translate("The email link is missing its token. Open the original invitation link again.")}</p>}<label><input type="checkbox" checked={policyAck} disabled={busy} onChange={e => setPolicyAck(e.target.checked)} />{translate("I understand the handoff and payment responsibilities.")}</label><button disabled={busy || !token || !policyAck} onClick={() => command('accept')}>{busy ? translate("Accepting…") : translate("Accept invitation and begin settlement")}</button></section>}
    {view && <section className="my-clouds-panel owner-handoff" aria-live="polite"><h2>{handoffTitle(view)}</h2><dl><dt>{translate("Cloud")}</dt><dd>{view.brand_cloud_id}</dd><dt>{translate("Transfer")}</dt><dd>{view.id}</dd><dt>{translate("Source owner")}</dt><dd>{view.source_user_id}</dd><dt>{translate("New owner")}</dt><dd>{view.target_email || view.target_user_id}</dd><dt>{translate("Phase")}</dt><dd>{view.phase} / {view.operation_phase || translate("invitation")}</dd></dl>
      <p>{translate("Transfer requires balance ≥ 0, settled usage and no unresolved payment, refund or dispute. A positive balance alone is insufficient.")}</p>
      <ul>{view.blockers.map((b,i) => <li key={`${b.code}-${i}`}>{blockerLabels[b.code] || b.code}</li>)}</ul>
      {safeSnapshot(view.balance_snapshot) ? <><h3>{translate("Settled balance snapshot")}</h3><p>{view.balance_snapshot.balance_minor} {translate("minor units (")}{view.balance_snapshot.currency}{translate(") · Ownership version")} {view.balance_snapshot.ownership_version} {translate("· Billing version")} {view.balance_snapshot.billing_snapshot_version}</p><p>{translate("Source confirmed:")} {view.source_confirmed ? translate("Yes") : translate("No")} {translate("· Target confirmed:")} {view.target_confirmed ? translate("Yes") : translate("No")}</p>{!ready && <p>{translate("This snapshot is historical or not currently confirmable. Do not treat it as permission to transfer.")}</p>}</> : <p>{translate("No exactly representable, confirmable settled snapshot is available.")}</p>}
      {ready && <><button disabled={busy} onClick={() => command('preview')}>{translate("Refresh settled balance")}</button>{previewKey === key && <><label><input type="checkbox" disabled={busy} checked={approval === key} onChange={e => setApproval(e.target.checked ? key : '')} />{translate("I confirm this exact balance and both versions, and accept the Billing handoff consequences.")}</label><button disabled={busy || approval !== key} onClick={() => command('confirm')}>{translate("Confirm exact balance")}</button></>}</>}
      {handoffCancelable(view,me?.user_id) && <><p>{translate("If blocked by debt, cancel first and wait for confirmed hold release before topping up. Do not pay inside a fenced handoff.")}</p><label><input type="checkbox" checked={cancelAck} disabled={busy} onChange={e => setCancelAck(e.target.checked)} />{translate("Request precommit cancellation; wait for all service holds to release.")}</label><button disabled={busy || !cancelAck} onClick={() => command('cancel')}>{translate("Request cancellation")}</button></>}
      {view.phase === 'finalizing' && <p>{translate("The unique owner has switched. Recovery must finish forward; there is no switch-back action. The new owner must set up their own payment method and consent after finalization.")}</p>}
      {handoffComplete(view) && (me?.user_id === view.target_user_id ? <a href={cloudURL(view.brand_cloud_id)}>{translate("Open your cloud")}</a> : <p>{translate("You no longer have access to this cloud or its Products.")}</p>)}
    </section>}
  </main></div>;
}
