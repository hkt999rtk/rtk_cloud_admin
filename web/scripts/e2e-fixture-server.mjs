...
const server = createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === '/simulator-newebpay' && req.method === 'POST') {
      res.writeHead(200, { 'Content-Type': 'text/html; charset=utf-8', 'Cache-Control': 'no-store' });
  ...
  return [{
    ...
    sdk_releases: [
      { name: 'Ameba Arduino Pro2', version: provider.snapshot_version, summary: 'Arduino SDK and examples for AmebaPro2.', recommended: true, supported_models: ['AMB82 MINI'], endpoints: [
        { type: 'github', title: 'Ameba Arduino Pro2 GitHub', url: 'https://github.com/Ameba-AIoT/ameba-arduino-pro2', source: 'official', languages: ['en'], verified_at: '2026-08-28' },
        { type: 'getting_started', title: 'Getting Started', url: 'https://ameba-doc-arduino-sdk.readthedocs-hosted.com/en/latest/ameba_pro2/amb82-mini/index.html', source: 'official', languages: ['en'], verified_at: '2026-08-28' },
      ] },
      { name: 'Ameba FreeRTOS Pro2', version: 'main', summary: 'FreeRTOS-based AmebaPro2 SDK.', recommended: false, supported_models: ['AMB82 MINI'], endpoints: [
        { type: 'github', title: 'Ameba FreeRTOS Pro2 GitHub', url: 'https://github.com/Ameba-AIoT/ameba-rtos-pro2', source: 'official', languages: ['en'], verified_at: '2026-08-28' },
      ] },
    ],
  }];
}
...
async function handleCustomerResource(req, res, url) {
  ...
  if (suffix === '/billing/account' && req.method === 'GET') return send(res, 200, { account: { id: `account-${orgID}`, organization_id: orgID, currency: 'TWD', available_balance_minor: 1250, state: 'active', version: 8, created_at: '2026-08-01T08:00:00Z', updated_at: '2026-08-15T08:00:00Z' }, auto_topup: autoTopUp, payment_providers: [{ name: 'simulator', environment: 'simulated', capabilities: { hosted_setup: true, merchant_initiated_charge: true } }, { name: 'newebpay', environment: 'simulated', capabilities: { hosted_charge: true, status_query: true, webhook: true } }] });
  if (suffix === '/billing/summary' && req.method === 'GET') return send(res, 200, { account: { currency: 'TWD', available_balance_minor: 1250, state: 'active' }, current_period: { currency: 'TWD', total_minor: 842, period_start: '2026-05-01T00:00:00Z', period_end: '2026-06-01T00:00:00Z', lines: invoice.lines, estimated: true }, forecast: { state: 'available', projected_period_total_minor: 1135, projected_remaining_minor: 293, average_daily_cost_minor: 37, observation_days: 22, usage_through: '2026-05-23T00:00:00Z', confidence: 'medium', calculated_at: '2026-05-23T02:30:00Z' }, runway: { state: 'available', projected_days: 33, average_daily_cost_minor: 37, lookback_days: 22, confidence: 'estimated', calculated_at: '2026-05-23T02:30:00Z' }, latest_invoice: invoice, calculated_at: '2026-05-23T02:30:00Z' });
  if (suffix === '/billing/usage' && req.method === 'GET') return send(res, 200, { currency: 'TWD', subtotal_minor: 842, tax_minor: 0, total_minor: 842, period_start: '2026-05-01T00:00:00Z', period_end: '2026-06-01T00:00:00Z', usage_through: '2026-05-23T00:00:00Z', fact_count: 466, lines: invoice.lines, estimated: true });
  ...
  if (suffix === '/topups/checkout' && req.method === 'POST') return send(res, 202, { payment_intent: { id: 'intent-hosted', amount_minor: 300, currency: 'TWD', provider: 'newebpay', state: 'processing' }, duplicate: false, payment_action: { method: 'POST', url: `http://${req.headers.host}/simulator-newebpay`, fields: { MerchantID: 'SIMULATOR', TradeInfo: 'synthetic-encrypted-fixture', TradeSha: 'synthetic-digest', Version: '2.3' } } });
