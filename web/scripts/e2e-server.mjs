...
const server = spawn('go', ['run', './cmd/server'], { cwd: repoRoot, env: { ...process.env, GOWORK: 'off', PORT: process.env.E2E_APP_PORT || '18082', DATABASE_PATH: path.join(databaseDir, 'e2e.db'), ACCOUNT_MANAGER_BASE_URL: `http://127.0.0.1:${fixturePort}`, SDK_PORTAL_BASE_URL: `http://127.0.0.1:${fixturePort}`, BILLING_SERVICE_BASE_URL: `http://127.0.0.1:${fixturePort}`, BILLING_SERVICE_TOKEN: 'e2e-billing-service-token-000000000', VIDEO_CLOUD_BASE_URL: `http://127.0.0.1:${fixturePort}`, VIDEO_CLOUD_ADMIN_TOKEN: 'e2e-video-token', VIDEO_CLOUD_PROMETHEUS_BASE_URL: prometheusBaseURL, CLOUD_LOGGER_ENDPOINT: `http://127.0.0.1:${fixturePort}`, CLOUD_LOGGER_INGEST_TOKEN: 'e2e-log-token', CUSTOMER_PASSWORD_LOGIN_ENABLED: 'true' }, stdio: 'inherit' });
