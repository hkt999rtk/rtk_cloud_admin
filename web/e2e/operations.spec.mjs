...
test('[UI-CA-OPS-001] platform admin can open operations and service logs', async ({ page }) => {
  ...
  await page.getByRole('link', { name: 'Operations Log', exact: true }).click();
  ...
  await page.getByRole('link', { name: 'Service Logs', exact: true }).click();
  await expect(page.getByRole('heading', { name: 'Cloud Service Logs', exact: true })).toBeVisible();
