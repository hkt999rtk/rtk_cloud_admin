...
test('[UI-CA-STAGING-001] staging platform admin can read operations @staging', async ({ page }) => {
  ...
  await expect(page.getByRole('heading', { name: 'Operations Log', exact: true })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Platform Home', exact: true })).toBeVisible();
});
