...
test('[UI-CA-JOBS-001] batch job uses server scope, idempotency and result lifecycle @brand-fleet', async ({ page }, testInfo) => {
  ...
  const created = await page.request.post(jobsAPI, { headers, data: payload });
  ...
  const pause = await page.request.post(`${jobsAPI}/${encodeURIComponent(job.id)}/pause`, { headers: { 'Idempotency-Key': `pause-${job.id}` } });
  expect([202, 409]).toContain(pause.status());
  if (pause.status() === 202) expect((await pause.json()).job.id).toBe(job.id);
  const replay = await page.request.post(jobsAPI, { headers, data: payload });
  expect(replay.status()).toBe(202);
  expect((await replay.json()).idempotent_replay).toBeTruthy();
  const current = await page.request.get(`${jobsAPI}/${encodeURIComponent(job.id)}`);
  expect(current.ok()).toBeTruthy();
  if ((await current.json()).job.state === 'paused') {
    const resume = await page.request.post(`${jobsAPI}/${encodeURIComponent(job.id)}/resume`, { headers: { 'Idempotency-Key': `resume-${job.id}` } });
    expect(resume.status()).toBe(202);
  }
  await waitForJob(page, job.id, undefined, jobsAPI);
  const result = await page.request.get(`${jobsAPI}/${encodeURIComponent(job.id)}/result`);
