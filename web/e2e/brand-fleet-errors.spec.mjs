...
test.describe('Brandname source and error states', () => {
  test('empty source renders an empty state @brand-fleet @errors', async ({ page }) => {
  ...
  test('stale source keeps data and exposes freshness state @brand-fleet @errors', async ({ page }) => {
  ...
  test('source mode renders unavailable instead of empty @brand-fleet @errors', async ({ page }) => {
  ...
  test('customer-safe error does not expose upstream credentials @brand-fleet @errors', async ({ page }) => {
    test.skip(process.env.E2E_SCENARIO_MODE !== 'unavailable', 'run with E2E_SCENARIO_MODE=unavailable');
    await login(page, 'developer');
    await page.goto('/console/brand-e2e-01/overview');
