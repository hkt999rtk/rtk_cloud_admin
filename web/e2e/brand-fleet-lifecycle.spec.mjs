...
import { assertAsyncJobProgress, assertDownloadResult, waitForJob } from './fixtures/brand-fleet.mjs';

const jobsAPI = '/api/developer/brand-clouds/33333333-3333-4333-8333-333333333333/jobs';

async function createBatch(page, key, scope = { device_ids: ['dev-e2e-001', 'dev-e2e-003'], query: {}, excluded_device_ids: [] }) {
  const response = await page.request.post(jobsAPI, {
...
test.describe('Brandname job lifecycle', () => {
  test('[UI-CA-BATCH-001] partial failure keeps per-item results and creates a new retry attempt @brand-fleet @full', async ({ page }) => {
    ...
    await waitForJob(page, job.id, ['partial_failed'], jobsAPI);
    const failed = await page.request.get(`${jobsAPI}/${job.id}/result`);
    ...
    const retry = await page.request.post(`${jobsAPI}/${job.id}/retry`, { headers: { 'Idempotency-Key': `e2e-retry-${job.id}` } });
  ...
  test('[UI-CA-BATCH-002] pause, resume and cancel are valid state transitions @brand-fleet @full', async ({ page }) => {
    ...
    const pause = await page.request.post(`${jobsAPI}/${job.id}/pause`, { headers: { 'Idempotency-Key': `pause-${job.id}` } });
    expect([202, 409]).toContain(pause.status());
    if (pause.ok()) {
      await assertAsyncJobProgress(page, job.id, 'paused', jobsAPI);
      const resume = await page.request.post(`${jobsAPI}/${job.id}/resume`, { headers: { 'Idempotency-Key': `resume-${job.id}` } });
      expect(resume.status()).toBe(202);
      await waitForJob(page, job.id, undefined, jobsAPI);
    }
    const cancelJob = await createBatch(page, `e2e-cancel-${Date.now()}`);
    const cancel = await page.request.post(`${jobsAPI}/${cancelJob.id}/cancel`, { headers: { 'Idempotency-Key': `cancel-${cancelJob.id}` } });
    expect([202, 409]).toContain(cancel.status());
    if (cancel.ok()) await assertAsyncJobProgress(page, cancelJob.id, 'cancelled', jobsAPI);
  });
  ...
  test('[UI-CA-BATCH-003] completed result supports JSON and CSV download @brand-fleet @full', async ({ page }) => {
    ...
    await waitForJob(page, job.id, undefined, jobsAPI);
    await assertDownloadResult(await page.request.get(`${jobsAPI}/${job.id}/result`), 'application/json');
    await assertDownloadResult(await page.request.get(`${jobsAPI}/${job.id}/result?format=csv`), 'text/csv');
  });
});
